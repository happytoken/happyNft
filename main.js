...
Vue.prototype.formatCntDecimal = function(cnt, decimal) {
    var m = Math.pow(10, decimal);
    return Math.floor(cnt * m) / m;
}
...
Vue.prototype.toast = function(title, time) {
	uni.showToast({
		image: "/static/imgs/logo.png",
		icon: "none",
		position: "bottom",
		title: title,
		duration: time ? time : this.execRespDelay,
	});
...
Vue.prototype.confirm = function(title, content, showCancel, callback) {
	uni.showModal({
...
Vue.prototype.baseServer = "https://hld-api.ours.pro/";
