	...
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		...
		<div v-else class="input" :class="{placeholder: selectText === placeholder}" @click="onUpperClick" >
			<view class="flex justify-start align-center text-center">
				<view v-if="selectText==item.value" v-for="(item, index) in innerList" :key="index" class="margin-top-xs">
					<image :src="'../../../static/imgs/coins/' + item.img" style="width: 1rem; height: 1rem;"></image>
				</view>
				<view class="margin-left-xs">
					<view v-if="selectText==item.value" v-for="(item, index) in innerList" :key="index">
						{{storage("lang")=='zh'?item.name:item.value}}
					</view>
				</view>
			</view>
		</div>
			...
			>
				...
				>
					<view class="flex justify-start align-center text-center">
						...
						<view class="margin-left-xs">{{storage("lang")=='zh'?item.name:item.value}}</view>
					</view>
				</div>
		...
		computed:{
			...
			innerList(){    // 转换列表的数据格式
				...
				orginArr.forEach((val, index)=>{
					...
					let img = typeof val === 'object' && 'img' in val ? val.img : val;
					let name = typeof val === 'object' && 'name' in val ? val.name : val;
					arr.push({
						isActive: false,
						value: value,
						disabled: isDisabled,
...
<style scoped lang="less">
	...
	@hover-color: #7b7e83;       //边框的颜色
	@active-color: #863ff8;       //活动的颜色
